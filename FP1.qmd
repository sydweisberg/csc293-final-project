...
library(class)
X <- model.matrix(~sexuality + non_marriage_candidate + gender + from, data = stardew_data)

X <- cbind(X, playtime = stardew_data$playtime)
...
```{r}
# using random forest gives us 17%
library(randomForest)
set.seed(1)
train$marriage_candidate <- as.factor(train$marriage_candidate)
bag.stardew <- randomForest(marriage_candidate ~ ., data = train, mtry = 11, importance = TRUE)

yhat.bag <- predict(bag.stardew, newdata = test_data)
mean(yhat.bag == test_data$marriage_candidate)
```
```{r}
# backward selection
full.model <- multinom(marriage_candidate ~ ., data = train)
# stepAIC(full.model, direction = "backward")
selected_model <- multinom(marriage_candidate ~ gender + sexuality +relationship + hobbies + personality + season + from, data = train)
yhat.selected <- predict(selected_model, newdata = test_data)
mean(yhat.selected == test_data$marriage_candidate)
